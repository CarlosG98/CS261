...
/*
* This is an additional function to bst_size() which takes a root node. Recursively adds up all nodes in a bst.
*
* Params:
*   bst_node - should be the root node of a bst
*
* Return:
*   Should return the total number of nodes
*/
int size(struct bst_node* root){
  ...
  if(root == NULL){
    ...
    return 1 + size(root->left) + size(root->right); // add nodes as we travel down subtrees, 1 is included for root
  }
}
...
int bst_size(struct bst* bst) {
  ...
  return size(bst->root); //pass root node to recursive count function. 
...
int path_sum(int sum, struct bst_node* node){
  
  if(node == NULL){ //if tree is empty, return 0
    return 0;
  }
  if(node->left == NULL && node->right == NULL){ //if no children nodes
    if(node->val == sum){ //if current node equals current sum
      return 1; // return 1
    }else{
      return 0;
    }
  ...
  return path_sum(sum-node->val, node->left) || path_sum(sum-node->val, node->right);
...
int bst_path_sum(int sum, struct bst* bst) {
  ...
  return path_sum(sum, bst->root);

}



void leftmost(struct bst_iterator* it, struct bst_node* node){

  while(node != NULL){
    stack_push(it->stack, node);
    node = node->left;
  }
}
