...
void stack_from_queues_push(struct stack_from_queues* stack, int value) {
  ...
  if(queue_isempty(stack->q1) == 1){ //if stack one is empty
    queue_enqueue(stack->q1, value); //add to stack one
    while(queue_isempty(stack->q2) == 0){ //while stack 2 is NOT empty
      int val = queue_dequeue(stack->q2); //take from stack 2
      queue_enqueue(stack->q1, val); //add to stack 1
    }
  }else{
    queue_enqueue(stack->q2, value); //if stack 1 is not empty, add to stack 2 instead
    while(queue_isempty(stack->q1) == 0){ //while stack 1 is not empty
      int val = queue_dequeue(stack->q1); //take from stack 1
      queue_enqueue(stack->q2, val); //add to stack 2
    }
  }
}
...
int stack_from_queues_top(struct stack_from_queues* stack) {
  ...
  if(queue_isempty(stack->q1) == 1){
    val = queue_front(stack->q2);
  }else{
    val = queue_front(stack->q1);
  }
