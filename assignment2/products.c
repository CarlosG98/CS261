...
struct dynarray* create_product_array(int num_products, char** names, int* inventory, float* prices) {
  ...
  struct dynarray* prod_arr = dynarray_create();

  for(int i=0; i < num_products; i++){
    struct product* temp = create_product(names[i], inventory[i], prices[i]);
    dynarray_insert(prod_arr, -1, temp);
  } 
...
struct product* find_max_price(struct dynarray* products) {
  ...
  struct product* max = dynarray_get(products, 0);
  int length = dynarray_length(products);
  for(int i=0; i < length; i++){
    struct product* temp = dynarray_get(products, i);
...
struct product* find_max_investment(struct dynarray* products) {
  ...
  for(int i=0; i < length; i++){
    struct product* temp = dynarray_get(products, i);
    if((temp->inventory * temp->price) > (max->inventory * max->price)){
      max = temp;
    }
  }
...
void sort_by_inventory(struct dynarray* products) {
  ...
  for(int i=0; i < length; i++){
    for(int j=0; j < length - i - 1; j++){
        prod1 = dynarray_get(products, j);
        prod2 = dynarray_get(products, j+1);
        if(prod1->inventory > prod2->inventory){
          dynarray_set(products, j, prod2);
          dynarray_set(products, j+1, prod1);
        }
    }
  }
}
